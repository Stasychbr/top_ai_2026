...
\begin{itemize}
  ...
  \item НИРС, производственная практика, выпускная квалификационная работа.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\scriptsize
\textbf{Ролевой фокус программы:} Data Engineer (PL-2) и AI Architect (LLM-5).
\vspace{0.2em}
\begin{adjustbox}{max width=\textwidth,max totalheight=0.73\textheight, center}
...
\midrule
\textbf{PL-2 Java/Scala/Kotlin}\newline\emph{Роль: Data Engineer} &
...
\textbf{PL-2.1Б:} применяет основные библиотеки для решения рутинных задач в серверном программировании: ввод-вывод, применение простейших примитивов многопоточного программирования, интеграция с базами данных.\newline
\textbf{PL-2.2Б:} разрабатывает и поддерживает простые ETL алгоритмы в пайплайнах обработки данных \\
\midrule
\textbf{LLM-5 Промпт-инженерия}\newline\emph{Роль: AI Architect} &
...
\textbf{LLM-5.1Б:} знает концепцию промпта. Умеет составлять и структурировать промпт.\newline
\textbf{LLM-5.2Б:} использует простые последовательности промптов \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\midrule
Семестр & 4 &  \\
Контактная работа, всего & 54,3 &  \\
\quad Аудиторные занятия & 50 &  \\
\quad\quad Занятия лекционного типа & 16 & 8 лекций \\
\quad\quad Лабораторные занятия & 30 & 15 лабораторных \\
\quad Иная контактная работа & 4,3 &  \\
\quad\quad Контроль самостоятельной работы & 4 &  \\
\quad\quad Промежуточная аттестация & 0,3 &  \\
Самостоятельная работа, в том числе & 18 &  \\
\quad Командный проект & 4 &  \\
\quad Выполнение индивидуальных заданий & 12 &  \\
Контроль & 35,7 &  \\
\quad Подготовка к экзамену & 35,7 &  \\
\midrule
\textbf{Общая трудоёмкость} & \textbf{108} & \textbf{3 зачётные единицы} \\
\textbf{Форма контроля} & -- & \textbf{экзамен, командный проект} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{frame}
...
\begin{frame}{Материально-техническое обеспечение}
\footnotesize
\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
\textbf{Аудиторное и сетевое обеспечение}
\begin{itemize}
  \item компьютерный класс для лабораторных занятий;
  \item рабочее место преподавателя, проектор / экран;
  \item доступ в Интернет и GitHub-репозиторий курса;
  \item локальная БД или контейнеры Docker.
\end{itemize}

\textbf{Учебные материалы}
\begin{itemize}
  \item тестовые наборы CSV/JSON/XML;
  \item шаблоны README, чек-листы code review;
  \item методические указания к лабораторным и курсовому проекту.
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Программное обеспечение}
\begin{itemize}
  \item JDK 17+, IntelliJ IDEA Community Edition;
  \item Git, Maven / Gradle;
  \item PostgreSQL 16, JDBC-драйвер, HikariCP;
  \item Jackson, SLF4J + Logback;
  \item доступ к GigaChat для заданий по промпт-инжинирингу.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
